...
async def parse_team_statistics_and_matches(team_url, team_name):
    """
    Parse team statistics and match data from the given team URL.

    This function navigates to the team's page, waits for the page to load,
    extracts all script tags from the page content, and parses the JSON data
    from the JavaScript variable. The JSON data contains team statistics and
    match data.

    :param team_url: The URL of the team's page
    :param team_name: The name of the team
    :return: A tuple of two dictionaries: the first contains team statistics
             organized by category, and the second contains match data.
    """
...
async def scrape_team_links_and_statistics():
    ...
    extracts team statistics and match data, creates a directory for the team,
    ...
    async with async_playwright() as p:
        ...
        for i in range(min(20, count)):  # Process only 20 teams
            ...
            team_data, match_data = await parse_team_statistics_and_matches(team_url, team_name.strip())
